...
export default function DashboardStatsGrid() {
    ...
    const getcustomerList = async () => {
        try{
            ...
            const response1 = await axios.get("getInvoice");
            const response2 = await axios.get("getInvoice");
            const response3 = await axios.get("services");
            setLoading(false);
            setCustomerData(response.data.length);
            setReportData(response1.data.length);
            setInvoiceData(response2.data.length);
