...
export default function Invoice_Generate() {
  ...
  const [PoObjectId, setPoObjectId] = useState("")
  ...
  function getDueDateLabel(reminderDateStr) {
            ...
            <div className="flex w-[60rem] mb-6 md:grid-cols-3 mt-6">
              <div>
                ...
                {(poData.length !== 0) ?
                  ...
                  console.log( getDueDateLabel(value.reminderDate)); 

                    return (
                      <div className="flex items-center mb-2 flex-wrap" key={index}>
                        {/* Profile Name & Invoice badge */}
                        <div className='w-[21rem] flex items-center'>
                          <input
                            type="checkbox"
                          ...
                          <span className={`ml-2 mr-2 ${value.invoiceCreated ? 'text-gray-400' : 'text-black dark:text-white'}`}>
                            {value.profileName}
                          </span>
                          ...
                          <div className='text-red-500 text-xs ml-2'>{profileError}</div>
                        </div>

                        {/* PO Rate */}
                        <div className='w-[10rem] ml-2'>
                          <input
                            type="number"
                            id={`rate-${index}`}
                            name="rate"
                      ...
                      ${value.invoiceCreated ? 'opacity-60 cursor-not-allowed' : ''}`}
                            placeholder="PO Rate"
                          />
                        </div>

                        {/* PO Remark */}
                        <div className='w-[16rem] ml-2'>
                          <input
                            type="text"
                            id={`remark-${index}`}
                            name="remark"
                        ...
                        {value.reminderRequired === 'Yes' && !value.invoiceCreated && (
                          <div className="ml-3 flex items-center">
                            ...
                            bg-green-100 text-green-700 dark:bg-green-200 dark:text-green-800">
                                ✅ Reminded on {value.reminderDate}
                              </span>
                            ) : (
                              <>
                                <span className={`inline-flex items-center text-xs font-medium px-2 py-0.5 rounded 
                              ${value.invoiceCreated ? 'bg-gray-200 text-gray-500 dark:bg-gray-600 dark:text-gray-400'
                                    : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-200 dark:text-yellow-800'}`}>
                                  ⏰ Reminder set
                                </span>
                                {value.reminderDate && (
                                  <>
                                    <span className="ml-2 text-gray-600 text-xs dark:text-gray-300">
                                      📅 {value.reminderDate}
                                    </span>
                                    {getDueDateLabel(value.reminderDate) && (
